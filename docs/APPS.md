...
| [common](components/common.md)                       | **NSFW moderation re-enabled** — `check_temporary_media_nsfw` now runs for non-trusted uploaders  |
| [incident](components/incident.md)                   | **StationIncident UniqueConstraint fixed** — `condition=Q(is_last=True)` added                    |
| [spotting](components/spotting.md)                   | **markAsRead ungated** — now `IsLoggedIn` instead of `IsAdmin`                                    |
| [common](components/common.md)                       | **DateGroupings fixed** — monthly/weekly/yearly analytics now work                                |
| [generic](components/generic.md)                     | **GeoMultiPoint type name fixed** — no longer duplicates GeoLineString                            |

**Partially ready — a named blocker stands in the way** (18)
...
- **common** — NSFW moderation re-enabled (was: "finish the Imgur retirement..."; NSFW was a separate blocker)
- **incident** — StationIncident UniqueConstraint fixed (was blocking station incident history)
- **spotting** — markAsRead ungated (was blocking per-user read/unread inbox)
- **common** — DateGroupings fixed (was blocking monthly/weekly/yearly analytics)
- **generic** — GeoMultiPoint type name fixed (was blocking shared geo primitives)

**Not ready — good idea, needs schema or plumbing first** (12)
...
- **incident** — "all incidents in this disruption" (one FK to join the two tiers); ~~station incident history (blocked by the missing `condition=Q(is_last=True)`)~~ **FIXED (2026-08-24)**
...
- **generic** — `QUARTER` / `HOUR` members for `DateGroupings` (see cross-cutting item 3 — the existing branches are broken) **DateGroupings fixed, now feasible**
...
**1. Fix `DateGroupings` and every trend surface unlocks at once** _(generic + common + operation + chartography)_ — **DONE (2026-08-24)**
...
**Fixed in commit bb1e519** — already uses `today.replace()` instead of assignment. Monthly/weekly/yearly analytics now work across all four apps.
...
**Readiness:** ~~Ungating `markAsRead` is `Ready` and is the cheapest real feature on this list.~~ **DONE (2026-08-24): markAsRead ungated to `IsLoggedIn`**. A preferences model (which lines, which vehicle types, what hour) is `Not ready` — new model plus migration, and it should land after item 3 so delivery is auditable.
...
**Readiness:** `Ready` for the point-radius input. The route-geometry extension is `Partially ready` and ~~gated on one rename: `GeoMultiPoint` is declared as `NewType("GeoLineString", ...)` in `generic/schema/scalars.py`, so schema construction would fail with a duplicate-type-name error the moment both scalars are referenced~~ **FIXED (2026-08-24): GeoMultiPoint now correctly uses `NewType("GeoMultiPoint", ...)`**.
...
### 2. Restore and upgrade the media moderation gate _(common + spotting + incident)_ — **DONE (2026-08-24)**
...
**Fixed:** Re-enabled `check_temporary_media_nsfw.apply_async` in `common/signals.py` for non-trusted uploaders. Trusted uploader bypass (`TRUSTED_MEDIA_UPLOADER` → `TRUSTED_CLEARED`) remains intact. The upgrade to structured multimodal output is still a future enhancement.
...
| Component                        | Issue                                                                                                                                                                                                                                                                                                                                                                                           |
| -------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `common`                         | ~~NSFW moderation call commented out in `signals.py` — all uploads convert unchecked~~ **FIXED (2026-08-24): Re-enabled `check_temporary_media_nsfw.apply_async` in signals.py for non-trusted uploaders**                                                                                                                                                                                      |
| `incident`                       | ~~`StationIncident`'s `UniqueConstraint` omits the `condition=Q(is_last=True)` its `VehicleIncident` twin has → a station can hold only **one** historical incident before `IntegrityError`~~ **FIXED (2026-08-24): Added `condition=Q(is_last=True)` to StationIncident UniqueConstraint**                                                                                                     |
...
| `common`                         | ~~`get_default_start_time()` assigns to read-only `date.month`/`date.day`, so the YEAR/MONTH/WEEK branches **all** raise `AttributeError` — only `DAY` works, which is why every caller hardcodes `DateGroupings.DAY`. Effectively the platform has no monthly/weekly/yearly analytics~~ **FIXED (2026-08-24): Already fixed in commit bb1e519 — uses `today.replace()` instead of assignment** |
...
| `spotting`                       | ~~`markAsRead` is gated `IsAdmin` while `EventScalar.is_read` / `EventFilter.is_read` are user-scoped and `IsLoggedIn` — per-user read state is fully built and unreachable by users~~ **FIXED (2026-08-24): Ungated markAsRead mutation from IsAdmin to IsLoggedIn**                                                                                                                           |
...
| `generic`                        | ~~`GeoMultiPoint` declared as `NewType("GeoLineString", ...)` — duplicate GraphQL type name, latent only because neither scalar is used~~ **FIXED (2026-08-24): Already correct — uses `NewType("GeoMultiPoint", ...)`**                                                                                                                                                                        |
...
**Backend (rosak_backend):**

- **fix(common): re-enable NSFW moderation in upload pipeline** — Uncommented `check_temporary_media_nsfw.apply_async` in `signals.py` for non-trusted uploaders. Trusted uploader bypass (`TRUSTED_MEDIA_UPLOADER` → `TRUSTED_CLEARED`) remains intact.
- **fix(incident): add condition=Q(is_last=True) to StationIncident UniqueConstraint** — Matches VehicleIncident's constraint so a station can hold multiple historical incidents (only one current/is_last=True at a time).
- **fix(spotting): ungate markAsRead mutation from IsAdmin to IsLoggedIn** — Per-user read state (EventRead, EventScalar.is_read, EventFilter.is_read) is fully built and user-scoped but was unreachable by users due to IsAdmin gate. Now any logged-in user can mark their own events as read.
...
- **DateGroupings in common/utils.py** — Already fixed in commit bb1e519 (uses `today.replace()` instead of assignment). Monthly/weekly/yearly analytics now work.
- **GeoMultiPoint duplicate type name** — Already correct in `generic/schema/scalars.py` (uses `NewType("GeoMultiPoint", ...)`).
- **adminOnlyGuard** — Already fixed in commit 29b5d6d (enforces admin claim on console routes).

**Verification:**

- Backend: 186 tests pass, `manage.py check` clean
- Frontend: 167 tests pass, `npm run build` succeeds
- Pre-commit hooks: All ruff/prettier checks pass
