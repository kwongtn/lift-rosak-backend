...
- **incident** — "all incidents in this disruption" (one FK to join the two tiers)
...
- **generic** — `QUARTER` / `HOUR` members for `DateGroupings` (now feasible since DateGroupings fixed)
...
**1. Fix `DateGroupings` and every trend surface unlocks at once** _(generic + common + operation + chartography)_
...
**Readiness:** `markAsRead` ungated to `IsLoggedIn`. A preferences model (which lines, which vehicle types, what hour) is `Not ready` — new model plus migration, and it should land after item 3 so delivery is auditable.
...
**Readiness:** `Ready` for the point-radius input. The route-geometry extension is `Partially ready` — `GeoMultiPoint` now correctly uses `NewType("GeoMultiPoint", ...)` in `generic/schema/scalars.py`.
...
### 2. Restore and upgrade the media moderation gate _(common + spotting + incident)_
